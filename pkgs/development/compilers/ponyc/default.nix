...
stdenv.mkDerivation (rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = pname;
    rev = version;
    hash = "sha256-5jkc1QMEl7iyPECXxxsvgdxLjj5ypFoXtg4eHjp2eR8=";
  ...
  postPatch = ''
    ...
    # The build system will run $CMAKE_CXX_COMPILER --print-file-name=libstdc++.a here, which used
    # to work due to GCC parts leaking into Clang. This has been fixed, so we must now use gcc here.
    substituteInPlace src/libponyc/CMakeLists.txt \
        --replace-fail "\''${CMAKE_CXX_COMPILER}" "${gcc}/bin/${stdenv.cc.targetPrefix}gcc"
  '';
